...
The original dataset contains approximately 3 million tweets.
...
Historical retrieved conversations provide grounding and context rather than copying an old response verbatim.
...
* Historical similarity is too low.
...
* Similarity-threshold coverage: **96.5%**
* Examples with a good historical match: **193 / 200**

These retrieval figures should **not** be interpreted as retrieval accuracy.

The retrieval corpus and golden evaluation examples originate from the same underlying AmazonHelp dataset. Therefore, the retrieval evaluation is not a fully independent benchmark and may be optimistic.

In particular, a high similarity score only indicates that a similar historical message was found. It does not establish that:

* the retrieved example is the correct solution,
* the predicted intent is correct,
* the generated reply is correct,
* the customer issue was resolved, or
* the system would perform equally well on completely unseen data.

A fully independent retrieval benchmark would require a held-out retrieval corpus that is separated from the evaluation examples at the source-data level.
...
High-risk intents such as payment, account access, and cancellation are intentionally handled more conservatively.
...
Because the human review contains only 15 examples, these scores should be treated as an initial qualitative signal rather than a statistically strong estimate of overall response quality.

---

# 10. LLM-as-Judge

An LLM-as-judge evaluation script is included in:
...
However, the LLM judge could not be executed during this experiment because the API account had no remaining credits.

Therefore:

**No LLM-as-judge scores or human-LLM agreement numbers are reported.**

This is intentionally reported as a limitation rather than fabricating results.

With an API account containing available credits, the included script can be executed to complete this evaluation.
...
Many broad or ambiguous customer complaints contain words associated with missing or damaged products, causing the classifier to select this intent.
...
However, this number should **not** be presented as an accuracy metric.
...
Retrieval coverage only indicates that a sufficiently similar historical example was found according to the selected similarity threshold. It does not prove that:

* the retrieved example is the correct solution,
* the generated reply is correct,
* the customer issue was actually resolved, or
* the classifier predicted the correct intent.
...
Even this result has limitations because the golden set was manually labelled from a limited sample rather than being a large, statistically stratified benchmark.
...
If the processed training and golden files are available, run:
...
The repository is designed so that the main evaluation can be reproduced quickly using the processed training and golden data.
...
This would provide a more reliable measurement of retrieval quality and reduce the risk of overly optimistic retrieval results.
...
This would provide stronger evidence for response quality and potential human/LLM agreement.
...
* LLM-as-judge execution was blocked by API credit availability.
* No human-LLM agreement score is reported because the LLM judge did not successfully run.
...
The strongest aspect of the project is that it provides a complete, lightweight, and reproducible support-agent pipeline using classical machine learning and historical AmazonHelp conversations.
